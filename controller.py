...
from config import TAP_DURATION
...
class ControllerInput:
    ...
    def _get_gamepad_button(
        ...
        token = self._normalize_button_name(
            name
        )

        token = BUTTON_NAME_MAP.get(
            token,
            token,
        )
    ...
    def _get_fallback_key(
        ...
        return FALLBACK_BUTTON_MAP.get(
            token
        )

    def _use_analog_for_dpad(
        self,
        token: str,
        duration: float,
    ) -> bool:
        if not (
            self.use_gamepad
            and self.gamepad is not None
        ):
            return False

        if "DPAD" not in token:
            return False
    ...
    def press(
        ...
        if (
            ...
            button = self._get_gamepad_button(
                token
            )
        ...
        fallback = self._get_fallback_key(
            token
        )
    ...
    def tap(
        self,
        name: str,
        duration: float = TAP_DURATION,
    ) -> None:
        token = self._normalize_button_name(
            name
        )

        if self._use_analog_for_dpad(
            token,
            duration,
        ):
            return
