...
def get_vk_code(
    ...
    if (
        key_str.startswith("F")
        and key_str[1:].isdigit()
    ):
        f_num = int(
            key_str[1:]
        )
...
def find_hwnd_by_exe(
    exe_path: str,
):
    pids = _find_pids_for_exe(
        exe_path
    )
    ...
    # Process may have restarted and the cache is stale.
...
def is_target_or_program_window_focused(
    program_hwnd: int | None = None,
) -> bool:
    foreground = (
        user32.GetForegroundWindow()
    )
    ...
    if (
        program_hwnd
        and foreground == program_hwnd
    ):
        return True

    target_hwnd = find_hwnd_by_exe(
        get_target_exe()
    )

    return bool(
        target_hwnd
        and foreground == target_hwnd
    )
...
def get_virtual_key_code(
    ...
    # Unknown mapping should fail safely,
    # rather than silently becoming Space.
    return 0