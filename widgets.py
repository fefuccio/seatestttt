...
def load_custom_fonts() -> Tuple[str, str]:
    """
    Loads bundled .ttf fonts into Qt's memory database.

    Updates the global font variables and returns:
        (main_font_family, mono_font_family)
    """
    global FONT_FAMILY, FONT_MONO

    # Main font
    main_font_path = bundled_resource(
        "fonts",
        "Inter-Regular.ttf",
    )
    ...
    # Monospace font
    mono_font_path = bundled_resource(
        "fonts",
        "ShareTechMono.ttf",
    )
...
class AnimatedMenu(QMenu):
    """QMenu that slides in and fades in smoothly on show."""
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Polished: 180 -> 160 (Smoother, faster opening feel)
...
class _NewLogsBadge(QWidget):
    """Small pill shown at the bottom edge when new logs arrive
    ...
    def __init__(self, parent: QWidget) -> None:
        ...
        # Polished: 140 -> 120 (Faster appearance response)
...
class LogView(QPlainTextEdit):
    """
    High-performance log view that batches appends, fades edges
    only when there's content beyond them, and preserves scroll
    position for a user who has scrolled away from the bottom.
    """

    # Distance (px) over which a fade ramps from 0 -> full opacity.
    FADE_DISTANCE = 24
    ...
    def __init__(
        ...
        # No viewport margin reservation: text sits flush against
        # the natural line height with no permanent y-offset. Fades
        # are painted as an overlay on top of the viewport instead.
    ...
    # ------------------------------------------------------------
    # Public API (unchanged signatures — existing callers keep working)
    # ------------------------------------------------------------
    ...
    def replace_last_line(
        ...
        if was_pinned:
            ...
            # Preserve reading position — don't move the visible
            # cursor/viewport for someone who has scrolled up.
            pass
    ...
    def _insert_entry(
        ...
        # 140% proportional line height per spec 3.1 (natural line
        # height, 1.4-1.5x), zero extra per-entry margin.
    ...
    def _border_color_for(color: str) -> Optional[str]:
        # Errors/warnings get a left accent border in addition to
        # foreground color, per severity color-coding guidance.
        severity_colors = {
            "#EF4444": "#EF4444",
            "#FFB610": "#FFB610",
        }
    ...
    def _flush_buffer(self) -> None:
        ...
        # Count-based pruning (time-based pruning is handled by
        # callers that timestamp entries; this view enforces the
        # hard cap so a runaway session can't blow up memory).
        ...
        if was_pinned:
            ...
            # Do not steal scroll position. Let the user keep
            # reading; show the "new logs" indicator instead.
    ...
    def _prune_if_needed(self) -> None:
        # setMaximumBlockCount already enforces the 5000-entry cap
        # at the Qt level (oldest blocks are dropped automatically
        # on insert), so nothing further to do here — kept as an
        # explicit hook in case time-based pruning is added later.
        pass
    ...
    # ------------------------------------------------------------
    # Fade math (mirrors the spec's clamp-based ramp)
    # ------------------------------------------------------------
    ...
    def _update_fade_state(self) -> None:
        ...
        if max_scroll <= 0:
            ...
            new_top = _clamp(
                scroll_top / self.FADE_DISTANCE, 0.0, 1.0
            )
            new_bottom = _clamp(
                bottom_distance / self.FADE_DISTANCE, 0.0, 1.0
            )

        if (
            new_top != self._top_fade_opacity
            or new_bottom != self._bottom_fade_opacity
        ):
    ...
    def keyPressEvent(self, event) -> None:
        # Home/End/PageUp/PageDown/Arrow keys are handled natively
        # by QPlainTextEdit when it has focus; we only special-case
        # the copy-all shortcut (handled via QShortcut above) and
...
class PolishedButton(QPushButton):
    """
    Button with a subtle press squish, optional hover scale/icon
    rotation, and a default hover glow.
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._squish_anim = QPropertyAnimation(
            self,
            b"scaleFactor",
        )
        # Polished: 120 -> 100 (More tactile press feedback, less drag)
        self._squish_anim.setDuration(100)
        self._squish_anim.setEasingCurve(
            QEasingCurve.Type.OutCubic
        )

        self._rotate_anim = QPropertyAnimation(
            self,
            b"iconRotation",
        )
        # Polished: 220 -> 200 (Faster gear rotation)
        self._rotate_anim.setDuration(200)
        self._rotate_anim.setEasingCurve(
            QEasingCurve.Type.OutCubic
        )
        ...
        self._glow_anim = QPropertyAnimation(
            self,
            b"glow",
        )
        # Polished: 200 -> 180 (More responsive hover glow)
        self._glow_anim.setDuration(180)
        self._glow_anim.setEasingCurve(
            QEasingCurve.Type.OutCubic
        )
    ...
    def paintEvent(self, event):
        ...
        if (
            self._base_icon is None
            or self._base_icon.isNull()
        ):
            return
    ...
    def mousePressEvent(self, e):
        if self.isEnabled():
            ...
            self._squish_anim.setStartValue(
                self._scale
            )
    ...
    def enterEvent(self, e):
        if self.isEnabled():
            ...
            if self._hover_rotate != 0.0:
                ...
                self._rotate_anim.setStartValue(
                    self._icon_rotation
                )
                self._rotate_anim.setEndValue(
                    self._hover_rotate
                )
...
class SwitchButton(QWidget):
    ...
    def __init__(
        ...
        self._track.mousePressEvent = (
            self.mousePressEvent
        )
        self._track.mouseReleaseEvent = (
            self.mouseReleaseEvent
        )

        track_container = QHBoxLayout()
        track_container.setContentsMargins(
            0,
            0,
            0,
            0,
        ...
        # Polished: 150 -> 130 (Slightly quicker toggle movement)
        ...
        # Polished: 80 -> 70 (Tighter squash release)
        self._squash_anim.setDuration(70)
        self._squash_anim.setEasingCurve(
            QEasingCurve.Type.OutCubic
        )
        self._squash_anim.valueChanged.connect(
            self._on_squash
        )
    ...
    def mouseReleaseEvent(self, event) -> None:
        self._squash_anim.stop()
        self._squash_anim.setStartValue(
            self._knob_squash
        )
...
class HotkeyButton(PolishedButton):
    ...
    def __init__(
        ...
        super().__init__(
            current_key if current_key else "None",
            parent,
        )
        ...
        # Polished: 900 -> 800 (Faster pulse loop, feels more alert)
        self._listen_anim.setDuration(800)
        self._listen_anim.setEasingCurve(
            QEasingCurve.Type.InOutSine
        )
        self._listen_anim.setStartValue(0.35)
        self._listen_anim.setKeyValueAt(
            0.5,
            1.0,
        )
        self._listen_anim.setEndValue(0.35)
        self._listen_anim.setLoopCount(-1)
        self._listen_anim.valueChanged.connect(
            self._set_listen_glow
        )
...
class AnimatedCheckIndicator(QWidget):
    """
    Reusable custom-painted checkbox indicator.

    Uses QPropertyAnimation for smooth, reliable animation.
    """
    ...
    def __init__(
        ...
        # Property animation on 'progress'
        self._anim = QPropertyAnimation(self, b"progress", self)
        # Polished: 300 -> 280 (Slightly faster checkmark animation)
        self._anim.setDuration(280)
        self._anim.setEasingCurve(QEasingCurve.Type.InOutQuad)
        # The progress setter will call update() automatically

    def get_progress(self) -> float:
        return self._progress

    def set_progress(self, value: float) -> None:
        """Setter for the animated property."""
    ...
    def set_checked(
        ...
        Args:
            ...
            animate: if True, animate from current progress; if False, snap.
        """
        checked = bool(checked)

        # If the logical state is unchanged, we might still want to
        # ensure the visual state is correct (e.g., if an animation was
        # interrupted and progress is off).
        if self._checked == checked:
            if not animate:
                self._anim.stop()
        ...
        # Start animation from current progress to target
...
class AnimatedCheckBox(QWidget):
    ...
    def __init__(
        ...
        self._box = AnimatedCheckIndicator(
            colors,
            16,
        )
...
class FadeInDialog(QDialog):
    """
    Base dialog that fades in/out via native windowOpacity.
    """
    ...
    def __init__(
        ...
        # Polished: 200 -> 180 (Reduced dialog opening delay)
...
class AdminWarningDialog(FadeInDialog):
    ...
    def _build_ui(self) -> None:
        ...
        msg = QLabel(
            "Without them, window mode detection "
            "and keyboard hotkeys won't work in-game.\n"
        ...
        proceed_btn = PolishedButton(
            "Proceed Anyway"
        )
        proceed_btn.setObjectName(
            "ProceedBtn"
        )
        proceed_btn.setCursor(
            Qt.CursorShape.PointingHandCursor
        )
        ...
        restart_btn = PolishedButton(
            "Restart as Admin"
        )
        restart_btn.setObjectName(
            "RestartBtn"
        )
        restart_btn.setCursor(
            Qt.CursorShape.PointingHandCursor
        )
    ...
    def _apply_style(self) -> None:
        ...
        self.setStyleSheet(
            ...
            QPushButton#ProceedBtn:hover {{
                background-color: {C['border_hi']};
            }}

            QPushButton#ProceedBtn:pressed {{
                padding-top: 9px;
                padding-bottom: 7px;
            }}
...
class UpdatePromptDialog(FadeInDialog):
    def __init__(
        ...
        self.setWindowTitle(
            "Sea Angler Assist (Update Available)"
        )
        self.setModal(True)
    ...
    def _build_ui(self) -> None:
        ...
        msg = QLabel(
            "A new version of Sea Angler Assist is available."
        )
        msg.setObjectName(
            "DialogMessage"
        )
        ...
        details = QLabel(
            "Your settings will not be affected. Apply this update?"
        )
        ...
        skip_btn = PolishedButton(
            "Skip for Now"
        )
        skip_btn.setObjectName(
            "SkipBtn"
        )
        skip_btn.setCursor(
            Qt.CursorShape.PointingHandCursor
        ...
        apply_btn = PolishedButton(
            "Apply Update"
        )
        apply_btn.setObjectName(
            "ApplyBtn"
        )
        apply_btn.setCursor(
            Qt.CursorShape.PointingHandCursor
...
class CatchBurst(QWidget):
    """
    A non-interactive overlay that draws a quick
    celebratory burst on a parent widget.
    """
    ...
    def __init__(
        ...
        self._t = 0.0
        # Polished: 900 -> 850 (Slightly snappier celebration burst)
        self._duration_ms = 850

        self._ring_radius = 0.0
        self._ring2_radius = 0.0

        self._ring_color = QColor(
            self._accent
        )
        self._ring2_color = QColor(
            self._accent
        )
...
class Toast(QWidget):
    """
    A small floating notification that slides in
    from the right and fades out.
    """
    ...
    def __init__(
        ...
        # Polished: 220 -> 200 (Faster appearance)
        ...
        # Polished: 400 -> 350 (Faster dismissal, less lingering)
    ...
    def popup(
        ...
        # Polished: 300 -> 280 (Faster sliding motion)
...
class ToastManager:
    """
    Queue toasts so multiple messages stack
    ...
    def _drain(self) -> None:
        ...
        toast._fade_out.finished.connect(
            lambda t=toast:
                self._on_close(t)
        )
